...
// helpers
  ...
  function observe (value, asRootData) {
    ...
    if (hasOwn(value, '__ob__') && value.__ob__ instanceof Observer) {
      ...
      ob = new Observer(value);   // 观察 new Vue 中设置的 data 数据
    }
    if (asRootData && ob) {
      ob.vmCount++;
    }
  ...
  function mergeAssets (
    parentVal,          // constructor.components||directive||filter
    childVal,           // vm.components||directive||filter
    vm,                 // vm
    key                 // components || directive || filter
  ) {
...
// is needed to cater to all possible types of children values.
   ...
   */
  var Watcher = function Watcher (              // Watch 的用处： 更新模板 和 监听某个值的变化  line:2772 触发
    vm,
    expOrFn,
    cb,
    options,
  ...
  function proxy (target, sourceKey, key) { // vm, _data, name
  ...
  function initData (vm) {
    ...
    data = vm._data = typeof data === 'function'          // 拼接data数据
      ? getData(data, vm)
      : data || {};
    ...
    // observe data
    observe(data, true /* asRootData */);           // new Vue 中配置的data 数据
  }
...
// without getting yelled at by flow
  ...
  function initMixin (Vue) {
    Vue.prototype._init = function (options) {
      ...
      initEvents(vm);
      initRender(vm);
      callHook(vm, 'beforeCreate');  // 如果有设置beforeCreate的声明周期就触发
  ...
  function Vue$3 (options) {
    if ("development" !== 'production' &&          // 如果不是通过 new 关键字创建的实例对象就报错
  ...
  function initGlobalAPI (Vue) {
    ...
    initMixin$1(Vue);
    initExtend(Vue);
    initAssetRegisters(Vue);     // Vue.component、 Vue.directive
  }
  ...
  function createCompilerCreator (baseCompile) {
    return function createCompiler (baseOptions) {
      function compile (
        ...
        compiled.tips = tips;
        return compiled               // compiled = {ast: '生成抽象语法树', render: 'vue能理解的dom结构', ...}
      }
